...
<script>
  ...
  export default {
    ...
    async beforeMount() {
      const response = await axios.get('http://localhost:4202/venue/' + this.$route.params.venue + '/1')
    ...
    methods: {
      ...
      async pageTo(n) {
        ...
        const response = await axios.get('http://localhost:4202/venue/' + this.$route.params.venue + '/' + n)
        this.table = response.data.entries
        this.pages = response.data.pages
        this.page = n
      },
